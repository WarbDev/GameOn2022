...
public class UIPlayerEvents : MonoBehaviour
{
    ...
    private void Start()
    {
        player.GetComponent<DamageableWithHealthComponent>().DamageAnimationPlayed += ((health)=>HealthChanged?.Invoke(health));
        player.GetComponent<Health>().Healed += ((health) => HealthChanged?.Invoke(health));
